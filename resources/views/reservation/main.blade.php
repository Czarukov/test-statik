<x-layout.reservation>
    @if(session('success'))
        <div class="alert alert-success w-full max-w-[500px] min-h-[30px] bg-lime-800 flex rounded-lg p-4 flex-col gap-4 mb-4">
            {{ session('success') }}<br>
            Reservering datum: {{ session('reservation')->date }} {{ session('reservation')->timeslot }}<br>
            Aantal bezoekers: {{ session('visitor_count') }}
        </div>
    @endif
    <form class="w-full max-w-[500px] min-h-[30px] dark:bg-stone-800 bg-stone-50 flex rounded-lg p-4 flex-col gap-4"
        ...
        <div>
            ...
            <select 
                id="timeslot" 
                name="timeslot" 
                class="border border-gray-500 rounded px-3 py-2 w-full mt-1 bg-stone-50 dark:bg-stone-800"
                required
            >
                @foreach ($timeslots as $slot)
                    <option value="{{ $slot }}" @selected(old('timeslot') == $slot)>{{ $slot }}</option>
                @endforeach
            </select>
            @error('timeslot')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
        ...
        <div>
            ...
            <div id="holder-visitors">
                ...
                @php
                    $oldVisitors = collect(old())
                        ->filter(fn($value, $key) => str_starts_with($key, 'subscription-'))
                        ->keys();
                    $visitorIDs = $oldVisitors->map(fn($key) => (int) str_replace('subscription-', '', $key))->unique();
                @endphp

                @foreach($visitorIDs as $visitorID)
                    {!! view('components.visitor-form', ['visitorID' => $visitorID])->render() !!}
                @endforeach
                @error('visitor')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
            @enderror
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const existingVisitors = holder.querySelectorAll('.visitor-form');
            existingVisitors.forEach(visitor => {
                const id = parseInt(visitor.id.replace('visitor-', ''));
                if (id > visitorCount) visitorCount = id;
                initializeVisitor(id);
            });
            ...
            function initializeVisitor(id) {
                ...
                const deleteBtn = document.querySelector(`#visitor-${id} .delete-visitor`);
                if (deleteBtn) {
